 ...
 * Seeds the Firestore database with initial data for journeySteps, faqs, and timelinePhases.
 *
 * Usage:
 ...
 *   - OR run with Firebase Admin default credentials
 */
...
/**
 * Seeds a Firestore collection with documents
 * @param {string} collectionName
 * @param {Array} documents
 * @param {string} [idField] - Field name to use as document ID
 */
async function seedCollection(collectionName, documents, idField = 'id') {
  const batch = db.batch();

  for (const doc of documents) {
    const docId = doc[idField] || db.collection(collectionName).doc().id;
...
async function seed() {
  ...
  try {
    await seedCollection('journeySteps', journeySteps, 'id');
    await seedCollection('faqs', faqs);
    await seedCollection('timelinePhases', timelinePhases, 'id');
